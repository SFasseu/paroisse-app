...
<div class="card" style="max-width:800px">
    <div class="card-header fw-bold" style="background:#1A3A6B;color:#fff">
        <i class="bi bi-plus-circle me-2"></i>Créer un nouveau groupe de catéchèse
    </div>
    <div class="card-body">
        <form action="{{ route('sacrements.groupes.store') }}" method="POST">
            @csrf
            <div class="row g-3">
                <div class="col-md-8">
                    <label class="form-label fw-semibold">Nom du groupe <span class="text-danger">*</span></label>
                    <input type="text" name="nom" class="form-control @error('nom') is-invalid @enderror"
                           value="{{ old('nom') }}" placeholder="ex: Groupe Lumière — Niveau 1 2025" required>
                    @error('nom')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Année pastorale <span class="text-danger">*</span></label>
                    <input type="text" name="annee_pastorale" class="form-control @error('annee_pastorale') is-invalid @enderror"
                           value="{{ old('annee_pastorale', '2025-2026') }}" placeholder="2025-2026" required>
                    @error('annee_pastorale')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" name="capacite_max" class="form-control" value="{{ old('capacite_max') }}" min="1">
                </div>
                <div class="col-md-8">
                    <label class="form-label fw-semibold">Lieu de réunion</label>
                    <input type="text" name="lieu_reunion" class="form-control" value="{{ old('lieu_reunion') }}" placeholder="Salle Saint-Joseph, Bépanda">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Heure de réunion</label>
                    <input type="time" name="heure_reunion" class="form-control" value="{{ old('heure_reunion') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Jour de réunion</label>
                    <select name="jour_reunion" class="form-select">
                        <option value="">— Sélectionner —</option>
                        @foreach(['lundi','mardi','mercredi','jeudi','vendredi','samedi','dimanche'] as $j)
                        <option value="{{ $j }}" {{ old('jour_reunion')==$j?'selected':'' }}>{{ ucfirst($j) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <hr class="my-4">
            <h6 class="fw-bold mb-3" style="color:#1A3A6B"><i class="bi bi-person-badge me-1"></i>Catéchistes</h6>
            <div class="row g-2">
                ...
                <div class="col-md-6">
                    <div class="border rounded p-2 d-flex align-items-center gap-2">
                        <input type="checkbox" name="catechistes[]" value="{{ $cat->id }}"
                               ...
                               {{ in_array($cat->id, old('catechistes', [])) ? 'checked' : '' }}>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('sacrements.groupes.index') }}" class="btn btn-outline-secondary">Annuler</a>
            </div>
        </form>
    </div>
</div>
